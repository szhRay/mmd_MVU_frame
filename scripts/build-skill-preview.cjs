...
fs.writeFileSync(path.join(out, 'mvu-preview-fixture.json'), JSON.stringify(card, null, 2).replace(/<\/script>/gi, '<\\/script>') + '\n');
console.log('已生成仅供 tavern-mmd 本地仿真的消息仓夹具；正式导入仍使用 output/mvu-regex.json。');
