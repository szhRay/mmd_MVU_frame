...
作者交互只使用以下四个方法。每次调用都是一次独立事务：复制当前快照、执行一个操作、运行 `MVU_MODEL.derive`、按 Schema 校验并提交。操作、派生或校验失败时抛出错误，本次调用不提交任何变化；成功时同步返回已提交最终快照的深拷贝，并在随后异步持久化。

若作者按 [AUTHORING.md](AUTHORING.md) 第二级规则在 Schema 中使用确定性规范化，返回值是规范化并校验后的最终快照，可能与传入的 `value` 不完全相同。非法操作或路径会在 Schema 运行前失败，不能通过 Zod 修复；最终状态仍不合法时，本次调用仍不提交。
...
四个方法都要求变量系统已经就绪，且路径、结果和派生值必须满足当前 Schema。返回值表示本次变量提交成功，不表示异步持久化已经完成。连续调用多个方法会形成多个独立提交；后一次失败不会撤销此前已经成功的调用。
